...
function EnviosRecientes() { 
    ...
    const obtenerEnvios = async () => { 
        try { const res = await axios.get("http://localhost:5000/envios"); 
            setEnvios(res.data); 
        } catch (error) { 
            console.log("Error cargando envíos:", error); 
        } finally { setCargando(false); } }; 
        useEffect(() => { obtenerEnvios(); }, []); 
        ...
        const enviarFormulario = async (e) => { e.preventDefault(); 
            try {if (envioEditando) { 
                await axios.put(`http://localhost:5000/envios/${envioEditando}`, formData); 
            } else { 
                await axios.post("http://localhost:5000/envios", formData); 
            } obtenerEnvios(); 
            cerrarModal(); 
        } catch (error) { console.log("Error guardando envío:", error); 

        } 
    }; 
    const eliminarEnvio = async (id) => { if (!window.confirm("¿Seguro quieres eliminar este envío?")) return; try { 
        await axios.delete(`http://localhost:5000/envios/${id}`); 
        obtenerEnvios(); 
    } catch (error) { console.log("Error eliminando envío:", error); 
...
<div className="envios-recientes-container"> 
    ...
    {cargando ? ( 
        <p className="cargando">Cargando envíos...</p> ) : envios.length === 0 ? ( 
            ...
            {envios.map((e) => ( 
                <tr key={e.id_envio}> 
                    ...
                    <td> 
                        ...
                        <button onClick={() => eliminarEnvio(e.id_envio)}>
                            Eliminar
                        </button> 
                    </td> 
                </tr> 
                    ...
                    <div className="input-group"> 
                        ...
                        <div className="input-group"> 
                            ...
                            <div className="input-group"> 
                                ...
                                value={formData.ciudad} onChange={manejarCambio} /> 
                                </div> <div className="input-group"> 
                                    <label>Código Postal:</label> 
                                    <input type="text" name="codigo_postal" 
                                    value={formData.codigo_postal} 
                                    onChange={manejarCambio} /> </div> 
